    ...
    return (
        <div className="glass-strong" style={{ padding: "32px", background: "rgba(15, 23, 42, 0.4)" }}>
            ...
            <div style={{ marginBottom: 32 }}>
                <label style={{ display: "block", fontSize: "0.9rem", fontWeight: 600, color: "var(--text-secondary)", marginBottom: 8 }}>Select Authorized Patient</label>
                <select
            ...
            {selectedPatient && !loadingRecords && (
                <>
                    ...
                    <div className="glass" style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap", alignItems: "center", padding: "16px", borderRadius: 12 }}>
                        <span style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 700, letterSpacing: "0.08em", marginRight: 8 }}>FILTERS:</span>
                        <select className="input-field" style={{ padding: "8px 12px", width: "auto", background: "rgba(255,255,255,0.05)" }} value={filterYear} onChange={e => setFilterYear(e.target.value)}>
                            <option value="all" style={{ color: "#0f172a" }}>All Years</option>
                            {years.map(y => <option key={y} value={y} style={{ color: "#0f172a" }}>{y}</option>)}
                        </select>
                        <select className="input-field" style={{ padding: "8px 12px", width: "auto", background: "rgba(255,255,255,0.05)" }} value={filterSeverity} onChange={e => setFilterSeverity(e.target.value)}>
                            <option value="all" style={{ color: "#0f172a" }}>All Severities</option>
                            <option value="high" style={{ color: "#0f172a" }}>Critical</option>
                            <option value="medium" style={{ color: "#0f172a" }}>Moderate</option>
                            <option value="low" style={{ color: "#0f172a" }}>Mild</option>
                        </select>
                        <div style={{ marginLeft: "auto", fontSize: "0.85rem", color: "var(--text-secondary)", fontWeight: 500 }}>
                            {filtered.length} record{filtered.length !== 1 ? "s" : ""} found
                        </div>
                    </div>

                    {filtered.length === 0 ? (
                        <div className="glass" style={{ padding: "48px", textAlign: "center", color: "var(--text-secondary)", border: "1px dashed var(--border)", borderRadius: 12 }}>
                            <div style={{ marginBottom: 16 }}>
                                <svg width="40" height="40" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ margin: "0 auto", color: "var(--text-secondary)" }}><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
                            </div>
                            <p style={{ margin: 0, fontWeight: 500 }}>No medical records found for this patient.</p>
                        </div>
                    ) : (
                        <div style={{ position: "relative", paddingLeft: 20 }}>
                            <div style={{ position: "absolute", left: 7, top: 12, bottom: 12, width: 2, background: "rgba(255,255,255,0.1)", zIndex: 0 }} />
                                ...
                                {filtered.map(rec => {
                                    ...
                                    return (
                                        <div key={rec.id} style={{ display: "flex", gap: 16, position: "relative", zIndex: 1 }}>
                                            {/* Timeline Node */}
                                            <div style={{ width: 16, height: 16, borderRadius: "50%", background: style.dot, border: "3px solid var(--bg-primary)", boxShadow: `0 0 10px ${style.dot}`, marginTop: 4, flexShrink: 0 }} />

                                            {/* Content Card */}
                                            <div style={{ flex: 1, padding: "20px", cursor: "pointer", transition: "all 0.2s ease", background: expanded ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.03)", border: "1px solid", borderColor: expanded ? "rgba(255,255,255,0.2)" : "var(--border)", borderRadius: 12, backdropFilter: "blur(12px)" }}
                                                ...
                                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                                                    <div>
                                                        <div style={{ fontSize: "0.8rem", color: "var(--text-secondary)", fontWeight: 600, letterSpacing: "0.05em", marginBottom: 2 }}>
                                                    ...
                                                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                                        {rec.reportFileURL && (
                                                            <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: "0.75rem", color: "#e2e8f0", background: "rgba(255,255,255,0.1)", padding: "4px 8px", borderRadius: 4, fontWeight: 600 }}>
                                                        ...
                                                        <span style={{ padding: "4px 8px", borderRadius: 4, fontSize: "0.75rem", fontWeight: 700, background: style.bg === "#fef2f2" ? "rgba(239, 68, 68, 0.1)" : style.bg === "#fefce8" ? "rgba(234, 179, 8, 0.1)" : "rgba(34, 197, 94, 0.1)", color: style.dot, border: `1px solid ${style.bg === "#fef2f2" ? "rgba(239, 68, 68, 0.2)" : style.bg === "#fefce8" ? "rgba(234, 179, 8, 0.2)" : "rgba(34, 197, 94, 0.2)"}` }}>
                                                            {style.label}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                {/* Preview */}
                                                {!expanded && (
                                                    <div style={{ marginTop: 12, fontSize: "0.9rem", color: "var(--text-secondary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                                        <span style={{ fontWeight: 600, color: "#fff", marginRight: 6 }}>Diagnosis:</span>
                                                        {rec.diagnosis}
                                                    </div>
                                                )}
                                                ...
                                                {/* Expanded details */}
                                                {expanded && (
                                                    <div style={{ marginTop: 20, borderTop: "1px solid var(--border)", paddingTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                                                        <div>
                                                            <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--text-secondary)", letterSpacing: "0.08em", marginBottom: 6, textTransform: "uppercase" }}>Diagnosis</div>
                                                            <div style={{ fontSize: "0.95rem", color: "#e2e8f0", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{rec.diagnosis}</div>
                                                        </div>
                                                        <div>
                                                            <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--text-secondary)", letterSpacing: "0.08em", marginBottom: 6, textTransform: "uppercase" }}>Prescription</div>
                                                            <div style={{ fontSize: "0.95rem", color: "#e2e8f0", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{rec.prescription}</div>
                                                        </div>
                                                        {rec.reportFileURL && (
